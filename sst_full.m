...
for i=1:Ntest
    real_lds(i)  = Result_lds{i}(1,5);
    real_mlds(i) = Result_mlds{i}(1,5);
    real_dct(i)  = result_dct{i}(1,5);
    real_dwt(i)  = result_dwt{i}(1,5);
    real_dft(i)  = result_dft{i}(1,5);
    real(i)      = X{i+Ntrain}(1,5); 
end
subplot(1,1,1);
hold on;
T = [1:Ntest]+Ntrain; 
